...
public class ThymeLeafControllerTest {
    ...
    @Test
    public void accessToLogin() throws Exception {
    ...
    public void loginUser() throws Exception {
        this.mockMvc.perform(formLogin().user("kermit").password("kermit"))
                .andExpect(authenticated());
    }

    @Test
    public void loginManager() throws Exception {
        this.mockMvc.perform(formLogin().user("gonzo").password("gonzo"))
                .andExpect(authenticated());
    }
    ...
    public void loginUserAccessProtected() throws Exception {
        MvcResult mvcResult = this.mockMvc.perform(formLogin().user("kermit").password("kermit"))
    ...
    @Test
    public void loginUserAccessProtectedTasks() throws Exception {
        MvcResult mvcResult = this.mockMvc.perform(formLogin().user("kermit").password("kermit"))
                .andExpect(authenticated()).andReturn();

        MockHttpSession httpSession = (MockHttpSession) mvcResult.getRequest().getSession(false);

        this.mockMvc.perform(get("/tasks").session(httpSession))
                .andExpect(status().isOk());
    }

    @Test
    public void loginUserAccessProtectedTask() throws Exception {
        MvcResult mvcResult = this.mockMvc.perform(formLogin().user("kermit").password("kermit"))
                .andExpect(authenticated()).andReturn();

        MockHttpSession httpSession = (MockHttpSession) mvcResult.getRequest().getSession(false);

        this.mockMvc.perform(get("/tasks/123").session(httpSession))
                .andExpect(status().isOk());
    }

    @Test
    public void loginManagerUserAccessProtectedTasks() throws Exception {
        MvcResult mvcResult = this.mockMvc.perform(formLogin().user("gonzo").password("gonzo"))
                .andExpect(authenticated()).andReturn();

        MockHttpSession httpSession = (MockHttpSession) mvcResult.getRequest().getSession(false);

        this.mockMvc.perform(get("/manager").session(httpSession))
                .andExpect(status().isOk());
    }
